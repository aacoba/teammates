...
public final class CommentsLogic {
    ...
    public List<CommentAttributes> getCommentsForReceiverVisibleToInstructor(
        ...
        List<CommentAttributes> comments = new LinkedList<>();
        HashSet<String> commentsVisitedSet = new HashSet<>();
    ...
    public List<CommentAttributes> getCommentsForStudent(StudentAttributes student)
        ...
        List<CommentAttributes> comments = new ArrayList<>();
        HashSet<String> commentsVisitedSet = new HashSet<>();
    ...
    private List<String> getSectionStudentsEmails(List<StudentAttributes> studentsInTheSameSection) {
        List<String> sectionStudentsEmails = new ArrayList<>();
        for (StudentAttributes stu : studentsInTheSameSection) {
    ...
    private List<String> getTeammatesEmails(List<StudentAttributes> teammates) {
        List<String> teammatesEmails = new ArrayList<>();
    ...
    public Set<String> getRecipientEmailsForSendingComments(String courseId) throws EntityDoesNotExistException {
        ...
        Map<String, List<StudentAttributes>> teamStudentTable = new HashMap<>();
        Map<String, List<StudentAttributes>> sectionStudentTable = new HashMap<>();
    ...
    private Set<String> populateRecipientEmails(String courseId, List<StudentAttributes> allStudents,
        ...
        Set<String> recipientEmailsList = new HashSet<>();
    ...
    private void populateTeamSectionStudentTables(List<StudentAttributes> allStudents,
        ...
        for (StudentAttributes student : allStudents) {
            ...
            if (studentsInTheSameSection == null) {
                studentsInTheSameSection = new ArrayList<>();
    ...
    private void populateRecipientEmailsFromPendingResponseComments(
        ...
        Map<String, FeedbackResponseAttributes> feedbackResponsesTable =
                new HashMap<>();
        Map<String, Set<String>> responseCommentsAddedTable = new HashMap<>();
    ...
    private void populateRecipientEmailsFromPendingComments(List<CommentAttributes> sendingCommentsList,
        ...
        Map<String, Set<String>> studentCommentsAddedTable = new HashMap<>();
    ...
    private void populateRecipientEmailsForSection(Set<String> recipientEmailList, CourseRoster roster,
        ...
        if (pendingComment.isVisibleTo(CommentParticipantType.SECTION)) {
            populateRecipientEmailsForSectionWhenCommentVisibleToSection(recipientEmailList, roster, teamStudentTable, sectionStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        } else { //not visible to SECTION
            populateRecipientEmailsForSectionWhenCommentNotVisibleToSection(roster, teamStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        }
    }
    
    private void populateRecipientEmailsForSectionWhenCommentVisibleToSection(Set<String> recipientEmailList, CourseRoster roster, Map<String, List<StudentAttributes>> teamStudentTable, Map<String, List<StudentAttributes>> sectionStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        if (pendingComment.recipientType == CommentParticipantType.PERSON) {
            checkForPersonBeforeAddingRecipientEmailsForSection(recipientEmailList, roster, sectionStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        
        } else if (pendingComment.recipientType == CommentParticipantType.TEAM) {
            checkForTeamBeforeAddingRecipientEmailsForSection(recipientEmailList, teamStudentTable, sectionStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        
        } else if (pendingComment.recipientType == CommentParticipantType.SECTION) {
            checkForSectionBeforeAddingRecipientEmailsForSection(recipientEmailList, sectionStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        
        }
    }
    
    private void checkForPersonBeforeAddingRecipientEmailsForSection(Set<String> recipientEmailList, CourseRoster roster, Map<String, List<StudentAttributes>> sectionStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
    ...
    private void checkForTeamBeforeAddingRecipientEmailsForSection(Set<String> recipientEmailList, Map<String, List<StudentAttributes>> teamStudentTable, Map<String, List<StudentAttributes>> sectionStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
    ...
    private void checkForSectionBeforeAddingRecipientEmailsForSection(Set<String> recipientEmailList, Map<String, List<StudentAttributes>> sectionStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
    ...
    private void populateRecipientEmailsForSectionWhenCommentNotVisibleToSection(CourseRoster roster, Map<String, List<StudentAttributes>> teamStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        if (pendingComment.recipientType == CommentParticipantType.PERSON) {
            checkForPersonBeforePreventAddingRecipientEmailsForSection(roster, teamStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        
        } else if (pendingComment.recipientType == CommentParticipantType.TEAM) {
            checkForTeamBeforePreventAddingRecipientEmailsForSection(teamStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        
        } else if (pendingComment.recipientType == CommentParticipantType.SECTION) {
            checkForSectionBeforePreventAddingRecipientEmailsForSection(teamStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        
        }
    }
    
    private void checkForPersonBeforePreventAddingRecipientEmailsForSection(CourseRoster roster, Map<String, List<StudentAttributes>> teamStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        for (String recipientEmail : pendingComment.recipients) {
            StudentAttributes student = roster.getStudentForEmail(recipientEmail);
            if (student != null) {
    ...
    private void checkForTeamBeforePreventAddingRecipientEmailsForSection(Map<String, List<StudentAttributes>> teamStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        for (String team : pendingComment.recipients) {
            List<StudentAttributes> students = teamStudentTable.get(team);
            if (students != null) {
                for (StudentAttributes stu : students) {
    ...
    private void checkForSectionBeforePreventAddingRecipientEmailsForSection(Map<String, List<StudentAttributes>> teamStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        for (String section : pendingComment.recipients) {
            preventAddRecipientEmailsForSection(teamStudentTable, studentCommentsAddedTable,
                    commentId, section);
        }
    ...
    private void populateRecipientEmailsForTeam(Set<String> recipientEmailList, CourseRoster roster,
        ...
        if (pendingComment.isVisibleTo(CommentParticipantType.TEAM)) {
            populateRecipientEmailsForTeamWhenCommentVisibleToTeam(recipientEmailList, roster, teamStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        } else { //not visible to TEAM
            populateRecipientEmailsForTeamWhenCommentNotVisibleToTeam(roster, teamStudentTable, studentCommentsAddedTable, pendingComment, commentId);
        }
    }
    
    private void populateRecipientEmailsForTeamWhenCommentVisibleToTeam(Set<String> recipientEmailList, CourseRoster roster, Map<String, List<StudentAttributes>> teamStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        if (pendingComment.recipientType == CommentParticipantType.PERSON) {
            for (String recipientEmail : pendingComment.recipients) {
                StudentAttributes student = roster.getStudentForEmail(recipientEmail);
                if (student != null) {
    ...
    private void populateRecipientEmailsForTeamWhenCommentNotVisibleToTeam(CourseRoster roster, Map<String, List<StudentAttributes>> teamStudentTable, Map<String, Set<String>> studentCommentsAddedTable, CommentAttributes pendingComment, String commentId) {
        if (pendingComment.recipientType == CommentParticipantType.PERSON) {
            for (String recipientEmail : pendingComment.recipients) {
                StudentAttributes student = roster.getStudentForEmail(recipientEmail);
                if (student != null) {
